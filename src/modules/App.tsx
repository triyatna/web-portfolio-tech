...
import { Blog } from "../modules/sections/Blog";
  ...
  const navItems = useMemo(
    () =>
      [
        ...
        { id: "blog", label: "Blog", show: Boolean(data.blog?.enabled) },
    ...
    <>
      ...
      <div className="min-h-screen selection:bg-black/10 selection:text-current">
        ...
        <header className="sticky top-4 z-40">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between rounded-2xl border border-subtle bg-[color:var(--bg)]/55 backdrop-blur-xl shadow-md px-4 py-2">
              <a
                className="font-semibold tracking-tight hover:opacity-80 focus-ring rounded px-1"
                href="#hero"
              >
                Tri Yatna
              </a>
      ...
      <style>{`
        ...
        .nav-active::after {
          ...
          opacity: .9;
          animation: navScan 2.2s linear infinite;
        }
        @keyframes navScan {
        ...
        @media (prefers-reduced-motion: reduce) {
          .nav-active::after { animation: none !important; }
        }
      `}</style>
    </>
  );
